...
def run_cocotb(
    ...
    test_name, top_level,
    include_dirs=None,
    verilog_sources=None, vhdl_sources=None,
    parameters=None,
    testcase=None,
):
    # Check arguments
    ...
    assert not(verilog_sources and vhdl_sources), "Only one language is supported" 
    assert verilog_sources or vhdl_sources, "no RTL files where specified"
    ...
    if include_dirs:
        compile_args.append(f"-I{d}" for d in include_dirs)
